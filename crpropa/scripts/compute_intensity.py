...
# ----------------------------------------------------------------------------------------------------
def compute_intensity(Zs, Gmm, Rcut):
    ...
    for igal in range(len(galaxy_data)):
        ...
        spec += np.histogram(data[:,2], bins = ENERGY_EDGES, weights = w_sim(data[:,4]) * w_spec(data[:,4], Zs, Gmm, Rcut))[0] / (4 * np.pi * galaxy_data[igal][8])**2

    return spec 

# ----------------------------------------------------------------------------------------------------
def write_intensity(Zs, Gmm, Rcut):

    spec = compute_intensity(Zs, Gmm, Rcut)
