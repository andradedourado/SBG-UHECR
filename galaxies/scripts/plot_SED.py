...
def load_SED_measurements(galaxy):
    ...
    if galaxy in ('NGC4038', 'NGC4039'):        
        data = pd.read_csv(f"{DATA_DIR}/data_36_{galaxy}.csv")
    
    else:
        for igal in range(len(gal_data)):
            if galaxy == gal_data[igal][2]:
                break
        data = pd.read_csv(f"{DATA_DIR}/data_{(igal + 2):02d}_{galaxy}.csv")
...
def plot_SED(galaxy):
    ...
    E, SED = load_SED_measurements(galaxy)
    ...
    plt.savefig(f"{FIGURES_DIR}/SED_{galaxy}.pdf", bbox_inches = 'tight')
    plt.savefig(f"{FIGURES_DIR}/SED_{galaxy}.png", bbox_inches = 'tight', dpi = 300)    
    plt.show()
