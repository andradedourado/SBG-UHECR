...
def plot_Eg_vs_E_xchecks(izg, zg):
    ...
    plt.legend(title = 'Results', loc = 'upper left')
...
def plot_Eg_vs_z_xchecks(iE, E):
    ...
    if E != 1e17:
        data_AA = np.loadtxt(f"{REFERENCES_DIR}/AA_Eg_vs_z_E{int(np.log10(E))}.dat")
        plt.plot(data_AA[:,0], data_AA[:,1], label = 'AA')

    data_AC = np.loadtxt(f"{REFERENCES_DIR}/AC_Eg_vs_z.txt")
    data_LAD = np.loadtxt(f"{RESULTS_DIR}/Eg_vs_z_xchecks_E{int(np.log10(E))}.dat")
