...
T_IR = 3.5e-3 * eV_to_J / const.k    # K
T_OPT = 332.5e-3 * eV_to_J / const.k # K
...
# ----------------------------------------------------------------------------------------------------
def photon_density(eps): # Number of photons per unit volume and energy

    return 8 * np.pi / (const.h * const.c)**3 * eps**2 * (NORM_IR / (np.exp(eps/(const.k * T_IR)) - 1) + NORM_OPT / (np.exp(eps/(const.k * T_OPT)) - 1))  
...
if __name__ == '__main__':
    ...
    write_pairproduction_timescales(4, 2)
    write_pairproduction_timescales(14, 7)
    write_pairproduction_timescales(28, 14)
    write_pairproduction_timescales(56, 26)
