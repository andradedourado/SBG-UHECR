...
def plot_proton_timescales():
    ...
    plt.plot(np.log10(data_advection[:,0] * 1.e18), data_advection[:,1], label = 'Advection')
    plt.plot(np.log10(data_diff_1H[:,0] * 1.e18), data_diff_1H[:,1], label = f'Diffusion')
    plt.plot(np.log10(data_spal_1H[:,0] * 1.e18), data_spal_1H[:,1], label = f'Hadronic')
    plt.plot(np.log10(data_photohadronic_1H[0]), data_photohadronic_1H[1], label = f'Photohadronic')
    plt.yscale('log')
    plt.ylim([1.e2, 1.e7])
    plt.xlabel(r'$\log_{10}{({\rm Energy}/{\rm eV})}$')
    plt.ylabel(r'Timescales$\: \rm [yr]$')
    plt.legend(bbox_to_anchor = (0., 1.02, 1., .102), loc = 'lower left', ncols = 2, mode = "expand", borderaxespad = 0.)
    plt.savefig(f"{FIGURES_DIR}/proton_timescales.pdf", bbox_inches = 'tight')
    plt.savefig(f"{FIGURES_DIR}/proton_timescales.png", bbox_inches = 'tight', dpi = 300)
    plt.show()
